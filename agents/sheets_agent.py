...
def _build_combined_pnl(pnl_reports: list[dict]) -> dict:
    ...
    combined = {
        ...
        "costs": {
            ...
            "cogs":                 sum(p["costs"].get("cogs", 0)                 for p in pnl_reports),
            "ads":                  sum(p["costs"].get("ads", 0)                  for p in pnl_reports),
            "warehouse":            sum(p["costs"].get("warehouse", 0)            for p in pnl_reports),
            "payroll":              sum(p["costs"].get("payroll", 0)              for p in pnl_reports),
            "packaging":            sum(p["costs"].get("packaging", 0)            for p in pnl_reports),
            "other_expense":        sum(p["costs"].get("other_expense", 0)        for p in pnl_reports),
            "total_platform_costs": sum(p["costs"].get("total_platform_costs", 0) for p in pnl_reports),
            "total_business_costs": sum(p["costs"].get("total_business_costs", 0) for p in pnl_reports),
            "total_costs":          sum(p["costs"].get("total_costs", 0)          for p in pnl_reports),
        },
        "profit": {},
